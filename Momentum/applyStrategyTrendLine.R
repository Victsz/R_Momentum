...
summary(strat)
add.indicator(strategy = myStrgy,name = 'TrendLine',arguments = list(mkt = s, r = 0.02))
...
add.signal(strategy = myStrgy, name="sigComparison", 
                      arguments=list(columns=c('Close','nearUp.TrendLine.ind'), relationship="lte"),
                                     label="nearUpTrend")           

add.signal(strategy = myStrgy, name="sigFormula", 
                      arguments=list(formula="(aboveUpTrend & nearUpTrend)",cross = F),
                                     label="longEntry")
add.signal(strategy = myStrgy, name="sigComparison", 
           arguments=list(columns=c('High','upL.TrendLine.ind'), relationship="gte"),
           label="longExit")
add.signal(strategy = myStrgy, name="sigComparison", 
...
# 
# add.signal(strategy = myStrgy, name="sigThreshold", 
#            arguments=list(threshold = 1, column='longInitSig.TrendLine.ind', relationship="eq"),
#            label="longEntry")
# add.signal(strategy = myStrgy, name="sigThreshold", 
#            arguments=list(threshold = -1, column='longInitSig.TrendLine.ind', relationship="eq"),
#            label="longExit")
# 
# add.signal(strategy = myStrgy, name="sigThreshold", 
#            arguments=list(threshold = 1, column='shortInitSig.TrendLine.ind', relationship="eq"),
#            label="shortEntry")
# add.signal(strategy = myStrgy, name="sigThreshold", 
#            arguments=list(threshold = -1, column='shortInitSig.TrendLine.ind', relationship="eq"),
#            label="shortExit")
...
add.rule(strategy = myStrgy, name="ruleSignal", 
         arguments=list(sigcol="longStop", sigval=TRUE, orderqty="all", 
                        ordertype="market", orderside="long", replace=FALSE, prefer="Open"), 
         type="exit", path.dep=TRUE)
# add.rule(strategy = myStrgy , name="ruleSignal", 
